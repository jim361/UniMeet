...
export default function LeaderDashboardScreen() {
  ...
  const approveApplication = useClubApplicationStore((state) => state.approveApplication);
  const rejectApplication = useClubApplicationStore((state) => state.rejectApplication);
  const getApplicationsByClub = useClubApplicationStore((state) => state.getApplicationsByClub);

  const leaderApplications = getApplicationsByClub(club.id);

  const handleDecide = (applicationId: string, decision: 'approve' | 'reject') => {
    if (decision === 'approve') {
                      ...
                      <View style={styles.applicantTop}>
                        <View>
                          ...
                          <Text style={styles.applicantMeta}>
                            {application.applicantId} · {new Date(application.appliedAt).toLocaleDateString('ko-KR')}
                          </Text>
                        </View>
                        <Badge
                          label={
                            application.status === 'approved' ? '합격'
                      ...
                      {decided ? (
                        ...
                        <View style={styles.decisionRow}>
                          <TouchableOpacity
                            ...
                            onPress={() => handleDecide(application.id, 'approve')}
                          >
                            <Text style={styles.acceptText}>합격</Text>
                          </TouchableOpacity>
                          <TouchableOpacity
                            style={styles.rejectButton}
                            onPress={() => handleDecide(application.id, 'reject')}
                          >
...
const styles = StyleSheet.create({
  ...
  emptyText: { color: colors.inkMuted, fontSize: 14, textAlign: 'center', padding: 20 },
  list: { gap: 12 },
  applicantCard: {
